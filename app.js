...
app.get('/:league_short_name/games', (req, res) => {
    db.task(t=>{
      ...
      `, [league.id]),
        ...
        LIMIT 15
      `, [league.id]),
                    t.manyOrNone('SELECT id, name, short_name FROM league WHERE is_active=true ORDER BY name ASC')
                ])
            })
    })
        .then((data) => {
...
app.get('/players', (req, res) => {
    db.task(t=>{
        return t.batch([
            t.manyOrNone('SELECT * FROM player WHERE is_active=true ORDER BY created_at DESC'),
            t.manyOrNone('SELECT * FROM player WHERE is_active=false ORDER BY created_at DESC'),
            t.manyOrNone('SELECT id, name, short_name FROM league WHERE is_active=true ORDER BY name ASC')
        ]);
    })
        .then((data) => {
...
app.get('/players/:id', (req, res) => {
    db.task(t=>{
        return t.batch([
            t.one('SELECT * FROM player WHERE id=$1', [req.params.id]),
            t.manyOrNone('SELECT id, name, short_name FROM league WHERE is_active=true ORDER BY name ASC')
        ]);
    })
        .then((data) => {
...
app.get('/leagues', (req, res) => {
    db.task(t=>{
        return t.batch([
            t.manyOrNone('SELECT * FROM league WHERE is_active=true ORDER BY created_at DESC'),
            t.manyOrNone('SELECT * FROM league WHERE is_active=false ORDER BY created_at DESC')
        ]);
    })
        .then((data) => {
            res.render('leagues', {
...
app.get('/leagues/:id', (req, res) => {
    db.task(t=>{
        return t.batch([
            t.one('SELECT * FROM league WHERE id=$1', [req.params.id]),
            t.manyOrNone(`
